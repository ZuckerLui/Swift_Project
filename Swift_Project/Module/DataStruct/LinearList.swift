...
class Node: Copyable{
    ...
    required init(_ data: String, _ next: Node?) {
        self.data = data
        self.next = next
    }
}
...
class LinearList {
    ...
    // 根据下标查找
    func foundNode(at index: Int) -> Node? {
        if length == 0 {
            return nil
        } else if index == 0 {
            return head
    ...
    // 根据值查找
    func foundNode(_ value: String) -> [Node] {
        var result: [Node] = []
        var tempNode = head
        while let node = tempNode {
            if node.data == value {
    ...
    func appendToHead(_ value: String) {
        let newNode = Node.init(value, nil)
        if let first = head {
            newNode.next = first
            head = newNode
        } else {
    ...
    func reverse1(_ head: Node?) -> LinearList? {
